...
export async function agDocExt(req: Request, res: Response): Promise<any> {
    uploadArchivo(req, res, function (error) {
        if (error) {
            return res.status(501).json({error});
        }
        if (req.file.filename) {
            let nvoRuta = path.resolve(__dirname, '../public/uploads/docsExt/' + req.file.filename);
...
export async function obDocExt(req: Request, res: Response): Promise<any> {
    let archivoUrl = req.query.archivoUrl;
    return res.sendFile(path.resolve(__dirname, `../public/uploads/docsExt/${archivoUrl}`));
}
